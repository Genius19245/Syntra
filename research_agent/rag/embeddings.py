...
class VertexEmbedder:
    ...
    def embed_texts(
        ...
        *,
        ...
        for text in texts:
            clipped = (text or "").strip()[:MAX_EMBED_CHARS]
            if not clipped:
                vectors.append(None)
                continue
            ...
            if embeddings:
                ...
                values = [float(item) for item in raw] or None
...
def embed_texts(
    ...
    *,
    ...
    if owner is None:
        return [None] * len(list(texts))
    try:
        return owner.embed_texts(list(texts), task_type=task_type)
    except TypeError:
        return owner.embed_texts(list(texts))  # type: ignore[call-arg]
    except Exception:
        return [None] * len(list(texts))

...
class SlideDeck {
  ...
  static SlideDeck? tryParse(String? raw) {
    ...
    final root = _deckRoot(json);
    if (root == null) return null;
    final list = root['slides'];
  ...
  static Map<String, dynamic>? _deckRoot(Map<String, dynamic> json) {
    final slides = pickJsonField(json, const [
      'slides',
      'slide_deck',
      'slideDeck',
      'deck',
    ]);
    if (slides is List) return json;
    if (slides is Map) {
      final inner = Map<String, dynamic>.from(slides);
      if (inner['slides'] is List) return inner;
    }
    if (slides is String) {
      final parsed = extractJsonMap(slides);
      if (parsed != null) return _deckRoot(parsed);
    }
    return null;
  }

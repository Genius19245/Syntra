...
class _AgentRunScreenState extends State<AgentRunScreen> {
  ...
  void _mergePipeline(Map<String, String> into, Map<String, String> delta) {
  ...
  void _applyOriginFrom(String? raw) {
    if (raw == null || raw.trim().isEmpty) return;
    final parsed = ResearchOrigin.parse(raw);
  ...
  Future<void> _run() async {
    ...
    try {
      ...
      await for (final event in _client.runSse(
        ...
        _mergePipeline(completeByAuthor, event.stateDelta);
        _applyOriginFrom(event.stateDelta['research_package']);

        if (event.hasText) {
          final text = event.text!;
          _applyOriginFrom(text);
          if (event.partial) {
            _liveText =
      ...
      final sessionState = await _client.fetchSessionState(
        userId: userId,
        sessionId: sessionId,
      );
      if (sessionState != null) {
        for (final entry in sessionState.entries) {
          final encoded = encodePipelineValue(entry.value);
          if (encoded != null && encoded.trim().isNotEmpty) {
            completeByAuthor[entry.key] = encoded;
          }
        }
        _applyOriginFrom(encodePipelineValue(sessionState['research_package']));
      }

      if (_curriculum.trim().isEmpty) {
        _curriculum = completeByAuthor['curriculum_agent'] ??
            completeByAuthor['syntra_orchestrator'] ??
            completeByAuthor['curriculum_plan'] ??
            '';
      }
      ...
      _applyOriginFrom(pipeline.researchPackage);
      if ((pipeline.curriculum == null || pipeline.curriculum!.trim().isEmpty) &&
          pipeline.slides == null &&
          pipeline.lessonPlan == null) {
        throw AdkException(
      ...
      if (_curriculum.trim().isEmpty) {
        _curriculum = pipeline.curriculum ??
            '# Curriculum Plan\n\nLesson assembled from the teaching pack.';
      }
      final teacherPayload = <String, dynamic>{
        ...
        if (pipeline.researchPackage != null)
          'researchPackage': pipeline.researchPackage,

...
void main() {
  ...
  test('parses lesson_plan wrap and duration aliases', () {
    const raw = '''
{
  "lesson_plan": {
    "lesson_sequence": [
      {
        "step": 1,
        "title": "Start from a compass",
        "purpose": "Activate prior knowledge",
        "activity": "Show a compass",
        "estimated_minutes": 5,
        "difficulty": "foundation"
      }
    ]
...
''';
    final plan = LessonPlan.tryParse(raw);
    expect(plan, isNotNull);
    expect(plan!.steps.single.title, 'Start from a compass');

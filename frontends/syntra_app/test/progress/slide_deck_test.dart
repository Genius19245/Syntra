...
''';
  ...
  test('parses output_key wrap and current agent field names', () {
    const raw = '''
{
  "slides": {
    ...
    "slides": [
      {
        ...
        "visual_asset": {
          ...
          "educational_purpose": "Show water moving",
          "url": "https://example.test/osmosis.png"
        },
        "teacher_explanation": "Point at the membrane.",
        "estimated_minutes": 4
      }
    ]
  }
...
''';
    final deck = SlideDeck.tryParse(raw);
    expect(deck, isNotNull);
    expect(deck!.lessonTitle, 'Osmosis');
    expect(deck.slides.single.visualType, 'ai_generated');
    expect(deck.slides.single.visualAsset?.ready, isTrue);
    expect(deck.slides.single.teacherExplanation, 'Point at the membrane.');
    expect(deck.slides.single.estimatedMinutes, 4);
  });

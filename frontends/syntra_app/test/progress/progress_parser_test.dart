...
''';
  ...
  test('PipelineTexts reads existing SSE author keys only', () {
    ...
    expect(texts.researchPackage, 'must not be sent to a new agent');
  });

  test('PipelineTexts reads ADK output_key state for slides and plan', () {
    const plan = '{"lesson_sequence":[{"step":1,"title":"Open with a demo","purpose":"Start","activity":"Show it","estimated_minutes":6}]}';
    const deck = '{"slides":[{"title":"Induction","content":["A changing field induces an EMF"]}]}';
    final texts = PipelineTexts.fromAuthors({
      'lesson_plan': plan,
      'slides': deck,
      'curriculum_plan': 'curriculum here',
      'research_package': '{"claims":[{"claim":"A changing field induces an EMF"}],"research_method":{"rag_used":true,"web_used":false}}',
    });

    expect(texts.lessonPlan, plan);
    expect(texts.slides, deck);
    expect(texts.curriculum, 'curriculum here');
    expect(texts.researchPackage, contains('research_method'));
    expect(LessonPlan.tryParse(texts.lessonPlan)?.steps.single.title, 'Open with a demo');
    expect(SlideDeck.tryParse(texts.slides)?.slides.single.title, 'Induction');
  });

  test('PipelineTexts reads teacher specialist output keys', () {
    final texts = PipelineTexts.fromAuthors({
      'explanation': '# Explanation\n\nFlux is how much field passes through a surface.',
      'example': '# Example\n\nA coil and a moving magnet.',
      'adaptation': '{"action":"simplify","stay_on_step":true}',
      'interaction_agent': '# Student interaction\n\nClarify flux.',
    });

    expect(texts.explanation, contains('Flux is how much field'));
    expect(texts.example, contains('moving magnet'));
    expect(texts.adaptation, contains('simplify'));
    expect(texts.interaction, contains('Clarify flux'));
    expect(texts.hasTeaching, isTrue);
  });
}

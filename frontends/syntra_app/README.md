...
`./scripts/dev.sh` passes the same `--dart-define-from-file=frontends/syntra_app/firebase.defines.json` flag when that file exists.
...
Do not add `cloud_firestore` to this app. If you edit Firestore rules, keep `syntra/{workspaceId}/research_cache` and `topic_clusters` as deny-all for clients:
...
match /topic_clusters/{clusterId} {
  allow read, write: if false;
}
```

The Research Agent writes cache and cluster catalog via the Admin SDK on Cloud Run.

...
Strict verification, and Refresh cache. Do not assume an exam
board when none is named. Do not assume A-Level, Physics, or
any other default.
...
If the brief says "Refresh cache: yes", skip any exact cache
hit. plan_retrieval will return HYBRID or WEB_ONLY, not
RAG_ONLY. Pass refresh_cache true. Do not treat cache_exact
as true. Delegate to the Source Researcher, then store a
fresh package.
...
3. Call plan_retrieval with the question, level, board, and labelled
   subject. If Refresh cache is yes, pass refresh_cache true.
   Honour the returned mode:
   ...
   A stale TTL or Refresh cache: yes clears cache_exact so HYBRID
   or WEB can re-research the topic.
...
async def run_agent(input_data, **kwargs):
    ...
    return await run_adk_agent(
        research_agent, input_data, app_name="research_agent", **kwargs
    )

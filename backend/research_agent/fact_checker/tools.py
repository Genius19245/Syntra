...
def search_web(query: str, max_results: int = 8) -> dict[str, Any]:
    ...
    return _search_web_cached(query, max(1, int(max_results or 8)))


@lru_cache(maxsize=32)
def _search_web_cached(query: str, max_results: int) -> dict[str, Any]:
    results: list[dict[str, str]] = []
    errors: list[str] = []
    ...
    return {
        "success": False,
        "error": (" ".join(errors) if errors else "No independent evidence was found."),
        "query": query,
        "results": [],
        "result_count": 0,
    }
...
def fetch_page(url: str, max_chars: int = 18000) -> dict[str, Any]:
    ...
    return _fetch_page_cached(url, max(1, int(max_chars or 18000)))


def _read_html(url: str) -> tuple[str, str]:
    with _http().get(
        url, timeout=REQUEST_TIMEOUT, stream=True, headers=_html_headers()
    ) as response:
        response.raise_for_status()
...
@lru_cache(maxsize=32)
def _fetch_page_cached(url: str, max_chars: int) -> dict[str, Any]:
    try:
        html, final_url = _read_html(url)
        soup = BeautifulSoup(html, "html.parser")
...
def verify_claims(claims: list[str]) -> dict[str, Any]:
    ...
    def _verify(item: dict[str, Any]) -> dict[str, Any]:
        ...
        fetched_pages: list[tuple[dict[str, str], dict[str, Any]]] = []
        if len(candidates) == 1:
            fetched_pages = [
                (candidates[0], fetch_page(candidates[0]["url"], max_chars=8000))
            ]
        elif candidates:
            with ThreadPoolExecutor(max_workers=2) as pool:
                fetched = list(
                    pool.map(
                        lambda result: fetch_page(result["url"], max_chars=8000),
                        candidates,
                    )
                )
            fetched_pages = list(zip(candidates, fetched))
        for result, fetched in fetched_pages:
